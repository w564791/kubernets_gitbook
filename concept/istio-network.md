FROM https://zhaohuabing.com/post/2018-09-25-istio-traffic-management-impl-intro/#inbound-cluster

转载者有对部分文档改动,有相应的说明

# 前言

Istio作为一个service mesh开源项目,其中最重要的功能就是对网格中微服务之间的流量进行管理,包括服务发现,请求路由和服务间的可靠通信。Istio实现了service mesh的控制面，并整合Envoy开源项目作为数据面的sidecar，一起对流量进行
...
![img](../assets/pilot-architecture.png)
...
![img](../assets/traffic-managment-components.png)
...
![img](../assets/envoy-config-init.png)
...
![img](../assets/envoy-config-bootstrap.png)
...
![img](../assets/envoy-config-clusters.png)
...
```
# istioctl pc cluster details-v1-6764bbc7f7-2mvpc --direction 'inbound|9080||details.default.svc.cluster.local' -o json
[
    {
...
> 3. 根据转载者的实际测试,虚拟端口监听为TCP或HTTP完全在与service的spec.ports[*].name字段是否含有http字样,如果没有,则直接建立相应的ClusterIP:port的虚拟监听,如果有http字样,则按照相应的端口归类到0.0.0.0:port归类到虚拟监听器,在0.0.0.0:port虚拟监听器中,按照请求的domain进行相应的路由(此段话由本转载着书写,水平有限,有错误之处还请指出)
...
![img](../assets/envoy-traffic-route.png)
